...
func (c *Coordinator) check(wid, tid int32) bool {
	format := "mr-out-%d"
	if c.stage == MAP {
		log.Printf("check map task %d...\n", tid)
		format += "-%d"
		for i := 0; i < c.nreduce; i++ {
			file := fmt.Sprintf(format+"-%d", tid, i, wid)
			_, err := os.Stat(file)
			if os.IsNotExist(err) {
				return false
			}
			os.Rename(file, fmt.Sprintf(format, tid, i))
		}
