...
func save(kva []KeyValue, wid int32, tid int32, nr int) {
	format := "mr-out-%d-%d-%d"
	buckets := make([][]KeyValue, nr)
	for i := range buckets {
		buckets[i] = make([]KeyValue, 0)
	}
	for _, kv := range kva {
		b := ihash(kv.Key) % nr
		buckets[b] = append(buckets[b], KeyValue{kv.Key, kv.Value})
	}
	for i, bucket := range buckets {
		file := fmt.Sprintf(format, tid, i, wid)
